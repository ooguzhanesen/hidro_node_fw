...
float ADS1115_ReadCurrent_mA(uint16_t channel_mux, float shunt_res_ohm)
{
    float v = 0.0f;

    if (ADS1115_readSingleEnded(channel_mux, &v) == HAL_OK)
    {
        if (shunt_res_ohm <= 0.0f)
            return -2.0f;   // Hatalı parametre

        return (v / shunt_res_ohm) * 1000.0f;  // mA
    }

    return -1.0f; // ADC okuma hatası
}

float ADS1115_ReadDividedVoltage(uint16_t channel_mux, float R1_ohm, float R2_ohm)
{
    float v_adc = 0.0f;

    if (ADS1115_readSingleEnded(channel_mux, &v_adc) == HAL_OK)
    {
        if (R1_ohm <= 0.0f || R2_ohm <= 0.0f)
            return -2.0f;   // Hatalı parametre

        float gain = (R1_ohm + R2_ohm) / R2_ohm;
        return v_adc * gain/1000;
    }
